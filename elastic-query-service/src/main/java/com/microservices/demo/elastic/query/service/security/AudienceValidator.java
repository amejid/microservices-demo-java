...
public class AudienceValidator implements OAuth2TokenValidator<Jwt> {
	...
	public OAuth2TokenValidatorResult validate(Jwt jwt) {
		if (jwt.getAudience().contains(this.elasticQueryServiceConfigData.getCustomAudience())) {
			return OAuth2TokenValidatorResult.success();
		}
		else {
			OAuth2Error audienceError = new OAuth2Error("invalid_token",
					"The required audience " + this.elasticQueryServiceConfigData.getCustomAudience() + " is missing",
					null);
