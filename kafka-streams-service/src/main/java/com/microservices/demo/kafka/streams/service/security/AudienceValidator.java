...
@Qualifier("kafkaStreamsServiceAudienceValidator")
@Component
public class AudienceValidator implements OAuth2TokenValidator<Jwt> {
	...
	public AudienceValidator(KafkaStreamsServiceConfigData config) {
		this.kafkaStreamsServiceConfig = config;
	}

	public OAuth2TokenValidatorResult validate(Jwt jwt) {
		if (jwt.getAudience().contains(this.kafkaStreamsServiceConfig.getCustomAudience())) {
			return OAuth2TokenValidatorResult.success();
		}
		else {
			OAuth2Error audienceError = new OAuth2Error("invalid_token",
					"The required audience " + this.kafkaStreamsServiceConfig.getCustomAudience() + " is missing!",
					null);
